...
ViridianGymScriptPointers: ; 748e1 (1d:48e1)
	...
	dw InitLeaguePCBattle

ViridianGymScript0: ; 748eb (1d:48eb)
	ld hl, wRanFlag
	bit 6, [hl] ; set in viridian city script
	res 6, [hl]
	jr z, .skip
	ld bc, $101
	ld a, $36
...
.skipsetting
	...
	ld bc, (2 << 8) | HS_VIRIDIAN_GYM_GIOVANNI
	predef FlagActionPredef
	ld a, c
	and a
	jr z, ViridianGymScript3_74995
...
InitLeaguePCBattle:
	...
	set 6, [hl]
	set 7, [hl]
	ld hl, LeaguePCEndText
	ld de, LeaguePCEndText
...
LeaguePCText:
	...
	ld a, [wRematchFlag]
	bit 7, a
	ld hl, LeaguePCAfterText
	jr nz, .skip
	ld hl, LeaguePCBeforeText
	call PrintTextWithPCSound
	call YesNoChoice
	ld a, [wCurrentMenuItem]
	and a
	ld hl, LeaguePCNoText
	jr nz, .skip
	ld hl, LeaguePCYesText
	call PrintTextWithPCSound
	ld a, 5
	ld [W_VIRIDIANGYMCURSCRIPT], a
	jr .finish
.skip
	call PrintTextWithLogOffSound
.finish
	jp TextScriptEnd
	
PrintTextWithPCSound:
	push hl
...
LeaguePCBeforeText:
	TX_FAR _LeaguePCBeforeText
	db "@"
	
LeaguePCYesText:
	TX_FAR _LeaguePCYesText
	db "@"
	
LeaguePCNoText:
	TX_FAR _LeaguePCNoText
	db "@"
	
LeaguePCEndText:
	TX_FAR _EndRematchBattle_LeaguePC
	db "@"
	
LeaguePCAfterText:
	TX_FAR _LeaguePCAfterText
	db "@"
